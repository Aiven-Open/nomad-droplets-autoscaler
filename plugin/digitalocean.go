	...
	for i := int64(0); i < diff; i++ {
		...
		go func(i int) {
			...
			if template.secureIntroductionAppRole != "" &&
				...
				if err := generateTagForSecureIntroduction(ctx, template, droplet, t.vault, t.client.Tags()); err != nil {
					cancel(err)
					return
				}
			}
...
func generateTagForSecureIntroduction(
	...
	if network := droplet.Networks; network != nil && len(network.V4) > 0 {
		...
		if _, err := tags.TagResources(ctx, fmt.Sprintf("%v%v", template.secureIntroductionTagPrefix, wrappedSecretId), &godo.TagResourcesRequest{Resources: []godo.Resource{{ID: fmt.Sprintf("%v", droplet.ID), Type: "droplet"}}}); err != nil {
			return fmt.Errorf(
